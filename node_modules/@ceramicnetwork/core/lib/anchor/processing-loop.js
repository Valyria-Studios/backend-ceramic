...
export class ProcessingLoop {
    ...
    start() {
        const waitForAbortSignal = new Promise((resolve) => {
            if (__classPrivateFieldGet(this, _ProcessingLoop_abortController, "f").signal.aborted) {
                return resolve();
            }
            const done = () => {
                __classPrivateFieldGet(this, _ProcessingLoop_abortController, "f").signal.removeEventListener('abort', done);
                resolve();
            };
            __classPrivateFieldGet(this, _ProcessingLoop_abortController, "f").signal.addEventListener('abort', done);
        });
