...
export class Dispatcher {
    constructor(_ipfs, networkOptions, repository, _logger, _pubsubLogger, _shutdownSignal, enableSync, maxQueriesPerSecond, recon, tasks = new TaskQueue()) {
        this._ipfs = _ipfs;
        ...
        if (!process.env.CERAMIC_RECON_MODE) {
            const pubsub = new Pubsub(_ipfs, networkOptions.pubsubTopic, IPFS_RESUBSCRIBE_INTERVAL_DELAY, IPFS_NO_MESSAGE_INTERVAL, _pubsubLogger, _logger, tasks);
    ...
    async storeTimeEvent(car, streamId, controllers, eventHeight, model) {
        const cid = car.roots[0];
        const eventId = this.recon.enabled && model
            ? EventID.create(this.networkOptions.name, this.networkOptions.id, 'model', model.toString(), controllers[0], streamId.cid, eventHeight, cid)
            : undefined;
        await this.importCAR(car, eventId);
    }
    importCAR(car, eventId) {
        if (eventId) {
    ...
    async storeInitEvent(data) {
        try {
            ...
            const eventId = header.model && this.recon.enabled
                ? EventID.create(this.networkOptions.name, this.networkOptions.id, 'model', StreamID.fromBytes(header.model).toString(), header.controllers[0], cid, 0, cid)
                : undefined;
    ...
    async storeDataEvent(data, eventHeight, streamId, controllers, model) {
        try {
            const carFile = this.carFactory.build();
            const cid = this._addCommitToCar(carFile, data);
            const eventId = model && this.recon.enabled
                ? EventID.create(this.networkOptions.name, this.networkOptions.id, 'model', model.toString(), controllers[0], streamId.cid, eventHeight, cid)
                : undefined;
            await this.importCAR(carFile, eventId);
            Metrics.count(COMMITS_STORED, 1);
