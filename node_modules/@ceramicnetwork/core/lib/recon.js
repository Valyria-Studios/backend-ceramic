...
var _ReconApi_config, _ReconApi_logger, _ReconApi_sendRequest, _ReconApi_url, _ReconApi_initialized, _ReconApi_pollInterval, _ReconApi_eventsSubscription, _ReconApi_stopSignal;
import { Subject, defer, concatMap, expand, of, retry, timer, Observable, filter, takeUntil, ReplaySubject, } from 'rxjs';
...
export class ReconApi extends Observable {
    ...
    async init(initialCursor = 0) {
        ...
        if (__classPrivateFieldGet(this, _ReconApi_config, "f").feedEnabled) {
            __classPrivateFieldSet(this, _ReconApi_eventsSubscription, this.createSubscription(initialCursor).subscribe(this.feed$), "f");
        }
    }
    ...
    async put(event, opts) {
        if (!this.enabled) {
            __classPrivateFieldGet(this, _ReconApi_logger, "f").imp(`Recon: disabled, not putting event ${event.id}`);
            return;
        }
        const body = {
        ...
        try {
            ...
            __classPrivateFieldGet(this, _ReconApi_logger, "f").debug(`Recon: put event ${event.id}`);
        }
        catch (err) {
            __classPrivateFieldGet(this, _ReconApi_logger, "f").err(`Recon: failed to add event ${event.id} with error ${err}`);
            throw err;
        }
    }
    get enabled() {
    ...
    createSubscription(initialCursor) {
        return of({ events: [], cursor: initialCursor, first: true }).pipe(expand((prev) => {
            return timer(prev.first ? 0 : __classPrivateFieldGet(this, _ReconApi_pollInterval, "f")).pipe(concatMap(() => defer(async () => {
                const response = await __classPrivateFieldGet(this, _ReconApi_sendRequest, "f").call(this, __classPrivateFieldGet(this, _ReconApi_url, "f") + `/ceramic/feed/events?resumeAt=${prev.cursor}`, {
                    method: 'GET',
                });
                return {
                ...
                delay: (err) => {
                    __classPrivateFieldGet(this, _ReconApi_logger, "f").warn(`Recon: event feed failed, due to connection error ${err}; attempting to retry in ${__classPrivateFieldGet(this, _ReconApi_pollInterval, "f")}ms`);
...
_ReconApi_config = new WeakMap(), _ReconApi_logger = new WeakMap(), _ReconApi_sendRequest = new WeakMap(), _ReconApi_url = new WeakMap(), _ReconApi_initialized = new WeakMap(), _ReconApi_pollInterval = new WeakMap(), _ReconApi_eventsSubscription = new WeakMap(), _ReconApi_stopSignal = new WeakMap();
