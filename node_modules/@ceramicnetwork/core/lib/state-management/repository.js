...
import { AnchorStatus, EventType, StreamUtils, SyncOptions, UnreachableCaseError, SignatureUtils, } from '@ceramicnetwork/common';
import { ExecutionQueue } from './execution-queue.js';
import { RunningState } from './running-state.js';
import { distinctUntilKeyChanged, map, Observable, concatMap } from 'rxjs';
...
export class Repository {
    constructor(cacheLimit, concurrencyLimit, feed, recon, logger) {
        this.recon = recon;
        this.logger = logger;
    ...
    async init() {
        ...
        const cursor = (await __classPrivateFieldGet(this, _Repository_deps, "f").keyValueStore.exists(RECON_STORE_CURSOR_KEY, RECON_STORE_USECASE_NAME))
            ? await __classPrivateFieldGet(this, _Repository_deps, "f").keyValueStore.get(RECON_STORE_CURSOR_KEY, RECON_STORE_USECASE_NAME)
            : 0;
    ...
    async _updateStateIfPinned_safe(state$) {
    ...
    async handleReconEvents(response) {
        ...
        for (const event of events) {
            ...
            try {
                ...
                if (!genesisCommitData.commit.header.model) {
                    throw new Error(`Model not found in genesis commit header ${genesisCid.toString()} for eventId ${eventId.toString()} for cid ${eventId.event.toString()}`);
                }
        ...
        await __classPrivateFieldGet(this, _Repository_deps, "f").keyValueStore.put(RECON_STORE_CURSOR_KEY, cursor.toString(), RECON_STORE_USECASE_NAME);
    }
        ...
        for (let remainingRetries = APPLY_ANCHOR_COMMIT_ATTEMPTS - 1; remainingRetries >= 0; remainingRetries--) {
            try {
                if (witnessCAR) {
                    ...
                    await this.dispatcher.storeTimeEvent(witnessCAR, streamId, state$.state.metadata.controllers, height, state$.state.metadata.model);
