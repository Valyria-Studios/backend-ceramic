...
function getReferencedAccount(account, doc, context) {
    switch(account){
...
class SchemaBuilder {
                        ...
                        case 'set':
                            {
                                if (model.accountRelation.type !== 'set') {
                                    throw new Error(`Invalid reference ${alias} on account: referenced model ${reference.name} must use the SET account relation`);
                                }
                                ...
                                config[alias] = {
                                    ...
                                    args: {
                                        with: {
                                            type: new GraphQLNonNull(withInput)
                                        }
                                    },
                        ...
                        case 'account-set':
                            {
                                ...
                                if (withFields.length !== 0) {
                                    // The SET reference requires a dedicated input object to specify the set fields values
                                    const withInput = this._buildSetInputObjectType(reference.name, withFields, reference.property);
                                    args.with = {
                                        type: new GraphQLNonNull(withInput)
                                    };
                                }
    ...
    _resolveInterfaces(node, modelName, ids = []) {
    ...
    _buildDocumentObjectRelation(key, definitions, relation, objectFields) {
        ...
        switch(relation.source){
            ...
            case 'set':
                {
                    const model = _class_private_field_get(this, _def).models[modelAlias];
                    if (model.accountRelation.type !== 'set') {
                        throw new Error(`Invalid relation on field ${key}: referenced model ${modelAlias} must use the "set" account relation`);
                    }
                    const relationFields = model.accountRelation.fields;
                    const args = {
                        account: accountRelationArg
                    ...
                    return {
                        type: _class_private_field_get(this, _types)[modelAlias],
                        args,
                        resolve: async (doc, args, ctx)=>{
    ...
    _buildInputObjectType(name, fields) {
        ...
        if (isDocument) {
            ...
            for (const field of Object.values(fields)){
                if (!field.immutable && field.type !== 'meta' && field.type !== 'view' && !(field.type === 'reference' && field.refType === 'connection')) {
    ...
    _buildSetInputObjectType(name, withFields, relationField) {
        const objectFields = _class_private_field_get(this, _def).objects[name];
        if (objectFields == null) {
            throw new Error(`Object fields not found for model ${name}`);
        }
        const relationFieldName = relationField ? relationField[0].toUpperCase() + relationField.slice(1) : '';
        const inputName = `With${relationFieldName}${name}Input`;
        if (_class_private_field_get(this, _inputObjects)[inputName] == null) {
            _class_private_field_get(this, _inputObjects)[inputName] = new GraphQLInputObjectType({
                name: inputName,
                fields: ()=>{
                    const fields = {};
                    for (const fieldName of withFields){
                        ...
                        switch(field.type){
                            ...
                            case 'meta':
                            case 'view':
                                throw new Error(`Invalid account relation field ${fieldName} on model ${name}: unsupported type ${field.type}`);
                            case 'reference':
                                switch(field.refType){
                                    case 'connection':
                                        throw new Error(`Invalid account relation field ${fieldName} on model ${name}: unsupported connection reference`);
                                    case 'enum':
                                        type = _class_private_field_get(this, _types)[field.refName];
                                        break;
                                    case 'node':
                                    ...
                                    case 'object':
                                        {
                                            throw new Error(`Invalid account relation field ${fieldName} on model ${name}: unsupported object reference`);
                                        }
                                }
                                break;
                            default:
                        ...
                        if (type == null) {
                            throw new Error(`Invalid account relation field ${fieldName} on model ${name}: type not found`);
                        }
                        fields[fieldName] = {
        ...
        return _class_private_field_get(this, _inputObjects)[inputName];
    }
    _buildNodeMutations(queryFields, name, model) {
        switch(model.accountRelation.type){
            ...
            case 'set':
                {
                    const relationFields = model.accountRelation.fields;
                    _class_private_field_get(this, _mutations)[`set${name}`] = mutationWithClientMutationId({
                        name: `Set${name}`,
                        inputFields: ()=>({
                                content: {
                                    type: new GraphQLNonNull(_class_private_field_get(this, _inputObjects)[name])
            ...
            case 'single':
                ...
                break;
            default:
                throw new Error(`Unsupported account relation type to create mutations: ${model.accountRelation.type}`);
        }
        _class_private_field_get(this, _mutations)[`update${name}`] = mutationWithClientMutationId({
            ...
            inputFields: ()=>{
                ...
                const partialContentInput = _class_private_field_get(this, _inputObjects)[`Partial${name}`];
                if (partialContentInput != null) {
                    inputFields.content = {
                        type: new GraphQLNonNull(partialContentInput)
                    };
                }
                return inputFields;
            },
            ...
            mutateAndGetPayload: async (input, ctx)=>{
                ...
                return {
                    document: await ctx.loader.update(input.id, input.content, input.options)
                };
            }
        });
        _class_private_field_get(this, _mutations)[`enableIndexing${name}`] = mutationWithClientMutationId({
            ...
            outputFields: ()=>({
                    ...queryFields,
                    document: {
                        type: _class_private_field_get(this, _types)[name]
                    }
                }),
            mutateAndGetPayload: async (input, ctx)=>{
                if (ctx.ceramic.did == null || !ctx.ceramic.did.authenticated) {
                ...
                return {
                    document: await ctx.enableDocIndexing(input.id, input.shouldIndex)
                };
            }
        });
    }
